...
@section('container')
    ...
    <div class="table-responsive col-lg-9">
        <a href="/dashboard/aktivitas/create" class="btn btn-sm btn-primary mb-3">Tambah Aktivitas Baru</a>
        <table class="table table-striped table-sm">
          ...
          <tbody>
          @if ($aktivitas->count())
            @foreach ($aktivitas as $aktivis)
            <tr>
                <td>{{ $loop->iteration }}</td>
                <td>{{ $aktivis->nama_konten }}</td>
                {{-- <td><img src="{{ asset('storage/'.$sponsor->foto) }}" alt="">/td> --}}
                <td><img src="{{ asset('storage/'.$aktivis->foto) }}" style="width: 70px;" alt="{{ $aktivis->nama_konten }}"></td>
                <td>
                  @if ($aktivis->status == 0)
                ...
                <td>
                  @if ($aktivis->status == "1")
                    <form action="/dashboard/aktivitas/status/{{ $aktivis->id }}" method="post" class="d-inline">
                  ...
                  @endif
                    ...
                    <a href="/dashboard/aktivitas/{{ $aktivis->id }}" class="badge bg-warning" title="ubah data"><i class="fas fa-edit"></i></a>
                    <form action="/dashboard/aktivitas/delete/{{ $aktivis->id }}" method="post" class="d-inline">
                    @method('delete')
                    @csrf
